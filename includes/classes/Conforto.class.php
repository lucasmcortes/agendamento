	...
	class Conforto extends ConsultaDatabase {
			...
			public function PossibilidadeManutencao($vid) {
				...
				if ($manutencao['mid']!=0) {
					...
					$motivo = parent::competenteManutencao($manutencao['motivo']);
			...
			public function Possibilidade($vid) {
				...
				$devolucao_status = new DateTime($this->data_agora);
				$status = 'Disponível';
				$disponibilidade_competente[$this->data_de_hoje->format('Y-m-d')]['status'] = $status;
				$aid = 0;
				...
				if ($possibilidademanutencao['mid']!=0) {
					for ($i=$inicio_status;$i<=$devolucao_status;$i->modify('+1 day')) {
						$disponibilidade_competente[$i->format('Y').'-'.$i->format('m').'-'.$i->format('d')] = array(
				...
				$alugueis_do_competente = parent::ListaAlugueiscompetente($vid);
				if ($alugueis_do_competente[0]['aid']!=0) {
					foreach ($alugueis_do_competente as $aluguel) {
						$aid = $aluguel['aid'];
						...
						for ($i=$inicio_status;$i<=$devolucao_status;$i->modify('+1 day')) {
							...
							if (!in_array($i,$disponibilidade_competente)) {
							} // in array

						} // for
					} // foreach $alugueis_do_competente
				} // alugueis > 0

				// usa a disponibilidade de hoje pra colocar no status
				$status = $disponibilidade_competente[$this->data_de_hoje->format('Y-m-d')]['status']??$status;

				return $this->possibilidade = $possibilidade = array (
					'disponibilidade'=>$disponibilidade_competente,
					'status'=>ucfirst($status)
				);
			} // Possibilidade
			...
			public function RevisaoDezKm($vid) {
				...
				$kilometragens = parent::Kilometragens($competente['vid']);

				$revisoes_feitas = [];
				$revisoes_necessarias = [];
				$data_revisoes_necessarias = [];

				if ($competente['categoria']!=3) {
					// carro e utilitario
					if ( ($competente['km']>$configuracoes['rev_car_prev']) && ($competente['km']<$configuracoes['rev_car_limiar']) ) {
						// revisa a cada 10k
						$multiplo = $configuracoes['rev_car_prev'];
					} else if ($competente['km']>=$configuracoes['rev_car_limiar']) {
						// revisa a cada 7k
				...
				} else if ($competente['categoria']==3) {
					// moto
					// revisa a cada 1k
					$multiplo = $configuracoes['rev_moto'];
				} // qual categoria

				($competente['revisao']==0) ? $multiplo = $multiplo : $multiplo = $competente['revisao'];
				...
				if ($multiplo>0) {
					...
					$manutencoes = parent::competenteManutencoes($competente['vid']);
					if ($manutencoes[0]!=0) {
					...
					if (count($revisoes_feitas) != count($revisoes_necessarias)) {
						if ($dataMultiploRecente<=$data_revisao_recente) {
							...
							// fica subentendido que a vez mais recente que fez revisão no competente já revisou tudo de uma vezes
			...
			public function ModificacaoPossivel($aid,$comeco_mod,$conclusao_mod) {
				...
				$disponibilidade_competente = $this->Possibilidade($competente['vid']);
				$disponibilidade = $disponibilidade_competente['status'];

				// vê os dias reservados de todos os alugueis desse competente
				$consultaralugueis = parent::ListaAlugueiscompetente($competente['vid']);
				if ($consultaralugueis[0]['aid']!=0) {
					foreach ($consultaralugueis as $aluguel) {
							...
							if ($consultareserva['reid']!=0) {
								...
								if ($ativacao['ativa']=='S') {
									...
									while ($i<=$devolucao_aluguel) {
										$dias_reservados_do_competente[] = $i->format('Y-m-d');
										$i->modify('+1 day');
									} // for
								} // ativa
							} else {
						...
						} // se devolveu o competente
					} // foreach aluguel
				} // se tem aluguel do competente
				...
				// dias_reservados_do_competente = todos os dias reservados do competente
				$dias_reservados_do_competente = array_values(array_unique($dias_reservados_do_competente, SORT_REGULAR));
				...
				$dias_dessa_reserva =  array_intersect($dias_reservados_do_competente,$dias_originais);
				// modificacoes = todos os dias que vão ser os dias da reserva depois da modificacao de agora
				$dias_possiveis = array_intersect($dias_dessa_reserva,$dias_reservados_do_competente);
				// dias que são de outra reserva
				$dias_de_outra_reserva = array_diff($dias_reservados_do_competente,$dias_dessa_reserva);
			...
			public function ModificacaocompetentePossivel($aid,$vid) {
				$this->data_de_hoje = new DateTime(date_create()->format('Y-m-d H:i:s.u'));
				$dias_reservados_do_competente = [];
				$dias_originais = [];
				...
				// vê os dias reservados de todos os alugueis desse competente
				$consultaralugueis = parent::ListaAlugueiscompetente($vid);
				if ($consultaralugueis[0]['aid']!=0) {
					foreach ($consultaralugueis as $aluguel) {
						$consultadevolucao = parent::Devolucao($aluguel['aid']);
						if ($consultadevolucao['deid']==0) {
				...
				// dias_originais = dias que tavam reservados nessa reserva (para a qual está sendo solicitada a modificação)
				$dias_originais = array_values(array_unique($dias_originais, SORT_REGULAR));
				// dias dessa reserva que são de outra reserva do competente
				$dias_agendados = array_intersect($dias_originais,$dias_reservados_do_competente);
				$dias_agendados = array_values(array_unique($dias_agendados, SORT_REGULAR));

				return $this->possibilidade = $dias_agendados;
			} // ModificacaocompetentePossivel
			...
			public function ModificacaoManutencaoPossivel($mid,$comeco_mod,$conclusao_mod) {
				...
				$competente = parent::competente($manutencao['vid']);
				$disponibilidade_competente = $this->Possibilidade($competente['vid']);
				$disponibilidade = $disponibilidade_competente['status'];
			...
			public function AluguelPossivel($vid,$comeco,$conclusao) {
				...
				$dias_reservados_do_competente = array_values(array_unique($dias_reservados_do_competente, SORT_REGULAR));
				// dias dessa modificação que são de outra reserva
				$dias_agendados = array_intersect($dias_reservados_do_competente,$intencoes);
				$dias_agendados = array_values(array_unique($dias_agendados, SORT_REGULAR));
			...
			public function DiasDesejados($vid,$comeco,$conclusao) {
				$this->data_agora = date_create()->format('Y-m-d H:i:s.u');
				$this->data_de_hoje = new DateTime($this->data_agora);
				$dias_indisponiveis_do_competente = [];
				$dias_agendados = [];
				$intencoes = [];
								...
								if ($ativacao['ativa']=='S') {
									...
									if ( ($inicio_aluguel->format('Y-m-d H:i'))<($this->data_de_hoje->format('Y-m-d H:i')) ) {
										if ($consultareserva['confirmada']==1) {
											...
											$i=$inicio_aluguel;
											while ($i<=$devolucao_aluguel) {
												$dias_indisponiveis_do_competente[] = $i->format('Y-m-d');
												$i->modify('+1 day');
											} // for
										} // se confirmou
									} else {
				...
				// dias_reservados_do_competente + dias em manutenção
				$dias_indisponiveis_do_competente = array_values(array_unique($dias_indisponiveis_do_competente, SORT_REGULAR));
				// dias dessa modificação que são de outra reserva
				$dias_agendados = array_intersect($dias_indisponiveis_do_competente,$intencoes);
				$dias_agendados = array_values(array_unique($dias_agendados, SORT_REGULAR));
			...
			public function FormatoHora($hora) {
				$hora = str_split($hora);
				$padrao = '%%h%%';
				$hora = vsprintf(str_replace('%', '%s', $padrao), $hora);
			...
			public function DisponivelPeriodo($inicio,$devolucao) {
					$sql = "
						SELECT vid
						FROM competente
						WHERE competente.vid NOT IN (
								SELECT competente.vid
								FROM aluguel
								INNER JOIN competente
									ON competente.vid=aluguel.vid
								WHERE aluguel.inicio > ?
								AND aluguel.devolucao < ?
							)
							AND competente.vid NOT IN (
									SELECT reserva.reid
									FROM reserva
									INNER JOIN ativacao
					...
					if (count($disponivel)>=1) {
						foreach ($disponivel as $competente) {
							$resultado[] = array(
			...
			public function Residual($coid) {
				...
				if ($cobranca['tid']!=0) {
					...
					if ($cobranca['valor']-$pagamentosaluguel-$somaparciais<0) {
						$devolvercliente = $cobranca['valor']-$pagamentosaluguel-$somaparciais;
				...
				if ($residual<0) {
					$status = 'Devolvido ao cliente';
				} // residual negativo (devolver pro cliente)
			...
			public function Potencia($vid) {
				...
				($competente['potencia']==0) ? $potencia = 0 : $potencia = $competente['potencia'];
				($competente['potencia']==1) ? $potencia = 0 : $potencia = $competente['potencia'];
				if ($potencia!=0) {
					$potencia = str_split($competente['potencia']);
			...
			public function ExcedenteData($aid) {
				$aluguel = parent::AluguelInfo($aid);
				$competente = parent::competente($aluguel['vid']);
				$categoria = parent::competenteCategoria($competente['categoria']);
			...
			public function InicioExpediente() {
				$configuracoes = parent::Configuracoes();
				$inicio = str_pad($configuracoes['hora_inicio_expediente'].'00',4,'0',STR_PAD_LEFT);

				return $this->inicio = $inicio;
			} // InicioExpediente

			public function EncerramentoExpediente() {
				$configuracoes = parent::Configuracoes();
				$encerramento = str_pad($configuracoes['hora_encerramento_expediente'].'00',4,'0',STR_PAD_LEFT);

				return $this->encerramento = $encerramento;
			} // EncerramentoExpediente

			public function DuracaoAgendamento() {
				$configuracoes = parent::Configuracoes();
				$duracao = $configuracoes['duracao_intervalo_agendamento'];

				return $this->duracao = $duracao;
			} // DuracaoAgendamento

			public function horasAgendaveis() {
				$f = 0;
				$horarios = [];
				...
				$intervalos = array(
					'10'=>array('00','10','20','30','40','50'),
					'15'=>array('00','15','30','45'),
					'20'=>array('00','20','40'),
					'30'=>array('00','30'),
					'60'=>array('00')
				);

				$duracaoAgendamento = $this->DuracaoAgendamento();
				$agendamentosPorHora = 60/$duracaoAgendamento;

				foreach($horasNoDia as $hora) {
					$horaComposta = str_pad($hora.'00',4,'0',STR_PAD_BOTH);
					if ( ($horaComposta >= $this->InicioExpediente()) && ($horaComposta <= $this->EncerramentoExpediente()) ) {
						for ($i=0;$i<$agendamentosPorHora;$i++) {
							$f++;
							$fila[] = $f; // posição do agendamento no dia

							$horarios[] = $hora.$intervalos[$duracaoAgendamento][$i]; // horários para possíveis agendamentos
							$horariosExibicao[] = $hora.'h'.$intervalos[$duracaoAgendamento][$i]; // horários para possíveis agendamentos escritos pra exibição
						} // minutos
					} // hora do expediente
				} // horas

				return $this->horarios = array(
					'horarios'=>$horarios,
					'exibicao'=>$horariosExibicao,
					'fila'=>$fila
				);
			} // horasAgendaveis

			public function DisponivelSlot($agendamentoCompleto) {
				global $dominio;

				$this->data = date_create()->format('Y-m-d H:i:s.u');
				$this->data_de_hoje = new DateTime($this->data);

				$agendamentoCompletoExplode = explode('_',$agendamentoCompleto);
				$agendamentoCompletoData = explode('-',$agendamentoCompletoExplode[0]);
				$agendamentoCompletoHora = explode('-',$agendamentoCompletoExplode[1]);
				$horaAtual = new DateTime($agendamentoCompletoData[0].'-'.$agendamentoCompletoData[1].'-'.$agendamentoCompletoData[2].' '.$agendamentoCompletoHora[0].':'.$agendamentoCompletoHora[1].':00.000000');

				if ($this->data_de_hoje>$horaAtual) {
					$timeSlotClass = 'passou';
					$timeSlotLegenda = 'indisponível';
					$timeSlotIcone = 'indisponivel';
					$timeSlotIconeClass = '';
				} else {
					$timeSlotClass = 'disponivel';
					$timeSlotLegenda = 'disponível';
					$timeSlotIcone = 'adicionar';
					$timeSlotIconeClass = 'disponivelicone';
				} // define a classe do botao de slot de agendamento disponivel

				$agenda = "
	                                        <p class='".$timeSlotClass."'>".$timeSlotLegenda."</p>
	                                        <div class='horaicones'>
	                                                <span class='info agendaricone' aria-label='".$timeSlotLegenda."'><img id='add_".$agendamentoCompleto."' class='linhaicon ".$timeSlotIconeClass."' src='".$dominio."/img/".$timeSlotIcone."-preto.png'></img></span>
	                                        </div>
				";
