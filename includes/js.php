...
alturaHeader = $("#menutopwrap").outerHeight()+$("#horapainellinear").outerHeight();
<?php
...
$(document).ready(function() {
        $('#botoesbottomlinear').css({
                'height':$("#menutopwrap").outerHeight()
        });
        $('#botoesbottomlinear p').css({
                'line-height':$("#menutopwrap").outerHeight()/1.618+'px'
        });
        $('.conteudo').css({
                'padding-bottom':$("#menutopwrap").outerHeight()
        });
});
/* altura do conteudo relativa ao tamanho do menutopwrap */
...
function fechaFundamental() {
        $('#fechar').on('click', function() {
                ...
                $.ajax({url: '<?php echo $dominio ?>/painel/agendamento/unsetHora.inc.php'});
        });
}
/* fecha fundamental */
...
function altAssociado(lid) {
        $.ajax({
                type: 'POST',
                url: '<?php echo $dominio ?>/painel/clientes/includes/altassociado.inc.php',
                data: {cliente: lid},
...
function altEndereco(lid) {
        $.ajax({
                type: 'POST',
                url: '<?php echo $dominio ?>/painel/clientes/includes/altend.inc.php',
                data: {cliente: lid},
                success: function(altend) {
                        $('#vestimenta').html(altend);
                }
        });
}
...
function clienteFundamental(lid) {
        $.ajax({
                type: 'POST',
                url: '<?php echo $dominio ?>/painel/clientes/includes/linfo.inc.php',
                data: {cliente: lid},
                beforeSend: function() {
                        /*loadFundamental('<?php echo $dominio ?>/includes/carregandofundamental.inc.php');*/
                },
...
function clienteVestimenta(lid) {
        $.ajax({
                type: 'POST',
                url: '<?php echo $dominio ?>/painel/clientes/includes/linfovestimenta.inc.php',
                data: {cliente: lid},
                beforeSend: function() {
                        /*loadFundamental('<?php echo $dominio ?>/includes/carregandofundamental.inc.php');*/
                },
...
function verrg(rg) {
	$.ajax({
		type: 'POST',
		url: '<?php echo $dominio ?>/painel/clientes/includes/verrg.inc.php',
		data: {documento: rg},
...
function competenteFundamental(vid) {
        $.ajax({
                type: 'POST',
                url: '<?php echo $dominio ?>/painel/competentes/includes/vinfo.inc.php',
                data: {competente: vid},
                beforeSend: function() {
                        /*loadFundamental('<?php echo $dominio ?>/includes/carregandofundamental.inc.php');*/
                },
...
function verDoc(vid) {
	$.ajax({
		type: 'POST',
		url: '<?php echo $dominio ?>/painel/competentes/includes/verdoc.inc.php',
		data: {competente: vid},
		success: function(img) {
			$('#fundamental').html(img);
		}
...
function verFoto(vid) {
	$.ajax({
		type: 'POST',
		url: '<?php echo $dominio ?>/painel/competentes/includes/verfoto.inc.php',
		data: {competente: vid},
		success: function(img) {
			$('#fundamental').html(img);
		}
...
function calendarioPop(estagio,lugar,vid) {
        $.ajax({
                type: 'POST',
                url: '<?php echo $dominio ?>/includes/calendariocompetente.inc.php',
                data: {
                        calendario: estagio,
                        lugar: lugar,
                        competente: vid
...
function calendarioSuperior(competente,dia,mes,ano) {
        $.ajax({
                type: 'POST',
                url: '<?php echo $dominio ?>/includes/calendariopainelsuperior.inc.php',
                data: {
                        competenteAtual: competente,
                        diaCalendarioSuperior: dia,
                        mesCalendarioSuperior: mes,
                        anoCalendarioSuperior: ano
                },
                beforeSend: function() {
                        $('#horas').empty();
                        $('#diadehoje').empty();
                        $('#diadehoje').html('<div id=\"enviando\" style=\"display:inline-block;\"><div id=\"enviandospinner\"></div></div>');
                },
                success: function(respostaCalendario) {
                        dataRespostaCalendario = respostaCalendario['data'];
                        $('#diadehoje').html(respostaCalendario['data']);
                        $('#horas').html(respostaCalendario['agenda']);

                        sessionStorage.setItem('dataAnteriorDia', respostaCalendario['anterior']['dia']);
                        sessionStorage.setItem('dataAnteriorMes', respostaCalendario['anterior']['mes']);
                        sessionStorage.setItem('dataAnteriorAno', respostaCalendario['anterior']['ano']);

                        sessionStorage.setItem('dataAtualDia', dia);
                        sessionStorage.setItem('dataAtualMes', mes);
                        sessionStorage.setItem('dataAtualAno', ano);

                        sessionStorage.setItem('dataSuperiorDia', respostaCalendario['posterior']['dia']);
                        sessionStorage.setItem('dataSuperiorMes', respostaCalendario['posterior']['mes']);
                        sessionStorage.setItem('dataSuperiorAno', respostaCalendario['posterior']['ano']);
...
function setHoraAgendamento(competenteEscolhido,horarioDisponivel) {
        $.ajax({
                type: 'POST',
                dataType: 'html',
                async: true,
                url: '<?php echo $dominio ?>/painel/agendamento/setHora.inc.php',
                data: {
                        horario: horarioDisponivel,
                        competente: competenteEscolhido
                },
                beforeSend: function() {
                        $('#diadehoje').html('<div id=\"enviando\" style=\"display:inline-block;\"><div id=\"enviandospinner\"></div></div>');
                },
                success: function(agendar) {
                        loadFundamental('<?php echo $dominio ?>/painel/agendamento/agendar.inc.php');
                }
        });
}

function verAgendamento(aid) {
        $.ajax({
                type: 'POST',
                dataType: 'html',
                async: true,
                url: '<?php echo $dominio ?>/painel/agendamento/setAid.inc.php',
                data: {
                        agendamento: aid
                },
                beforeSend: function() {
                ...
                success: function(agendar) {
                        loadFundamental('<?php echo $dominio ?>/painel/agendamento/veragendamento.inc.php');
                }
        });
}

function desmarcarAgendamento(aid) {
	$.ajax({
		type: 'POST',
		url: '<?php echo $dominio ?>/painel/agendamento/desmarcaragendamentopopup.inc.php',
		data: {
			agendamento: aid
		},
		beforeSend: function() {
...
function dataDesejada() {
        $('.escolhido').on('click',function () {
                calendarioSuperior($('#competentepainellinear').val(),$(this).find('.day-number').attr('id').split('_')[1].split('-')[2],$(this).find('.day-number').attr('id').split('_')[1].split('-')[1],$(this).find('.day-number').attr('id').split('_')[1].split('-')[0]);
...
// pro aluguel novo preencher a diária configurada pra cada categoria de competente
function DiariaCategoriaSelecionada() {
        categoriaselecionado = $("#v_"+valcompetente+"").find(".categoriaslide").html();
...
function mostraMenuTop() {
        $('#menutopinnerwrap').css('display','flex');
        $('#menutopwrap').css('background-color','var(--cremelight)');
}

function hideMenuTop() {
        $('#menutopinnerwrap').css('display','none');
        $('#menutopwrap').css('background-color','transparent');
}
