				...
				<?php
					if (!empty($imagem)) {
						...
						if (strpos($imagem,'.pdf')!==false) {
							echo "
								<iframe id='docimg' src='".$dominio."/painel/competentes/doc/".$imagem."?".rand(1,999)."' style='width:100%;auto;'></iframe>
							";
								MontaBotao('ver pdf','dldoc');
							echo "
								<script>
									$('#dldoc').on('click', function() {
										window.open('".$dominio."/painel/competentes/doc/".$imagem."','_blank');
									})
								</script>
							";
						} else {
							echo "
								<img class='docimg' style='max-width:100%;max-height:100%;' src='".$dominio."/painel/competentes/doc/".$imagem."?".rand(1, 999)."'></img>
								<script>
						...
						echo "
							<script>
								$('#atualizaimgdoc').on('click', function () {
									loadVestimenta('".$dominio."/painel/competentes/novo/includes/atualizaimgdoc.inc.php');
								});
							</script>
							</div>
						";
			                        ...
			                        <div id='img_doc_outer_wrap' class='uploadouterwrap'>
			                                ...
			                                <script>
			                                        ...
			                                        function uploadFile(elemento) {
			                                                ...
			                                                ajax.open('POST','".$dominio."/painel/competentes/novo/includes/addimgdoc.inc.php');
			                                                ajax.send(formdata);
			                                        }
					                                ...
					                                $.ajax({
					                                        url: '".$dominio."/painel/competentes/novo/includes/salvaimgdoc.inc.php',
					                                        success: function(salvadoc) {
					                                                $('#docimg').attr('src','".$dominio."/painel/competentes/doc/'+salvadoc+'?".rand(1, 999)."');
					                                        }
					                                });
			                                                ...
			                                                $('#remove_img_doc').on('click',function() {
			                                                        ...
			                                                        $.ajax({
			                                                                url: '".$dominio."/painel/competentes/novo/includes/unsetdoc.inc.php'
			                                                        });
			                                                });
			                                        }
