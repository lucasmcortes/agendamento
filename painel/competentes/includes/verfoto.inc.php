...
<!-- items -->
<div class="items">
	<?php tituloCarro($competente['modelo']); ?>
				...
				<?php
					if (!empty($imagem)) {
						...
						if (strpos($imagem,'.pdf')!==false) {
							echo "
								<iframe id='fotoimg' src='".$dominio."/painel/competentes/foto/".$imagem."?".rand(1,999)."' style='width:100%;auto;'></iframe>
							";
								MontaBotao('ver pdf','dlfoto');
							echo "
								<script>
									$('#dlfoto').on('click', function() {
										window.open('".$dominio."/painel/competentes/foto/".$imagem."','_blank');
									})
								</script>
							";
						} else {
							echo "
								<img class='fotoimg' style='max-width:100%;max-height:100%;' src='".$dominio."/painel/competentes/foto/".$imagem."?".rand(1, 999)."'></img>
								<script>
						...
						echo "
							<script>
								$('#atualizaimgfoto').on('click', function () {
									loadVestimenta('".$dominio."/painel/competentes/novo/includes/atualizaimgfoto.inc.php');
								});
							</script>
							</div>
						";
						...
						echo "
						  <!-- img_foto_outer_wrap -->
  					        <div id='img_foto_outer_wrap' class='uploadouterwrap'>
  					                <label>Foto do competente:</label>
  					                ...
  					                <script>
  					                        ...
  					                        function uploadFoto(elemento) {
  					                                ...
  					                                ajax.open('POST','".$dominio."/painel/competentes/novo/includes/addimgfoto.inc.php');
  					                                ajax.send(formdata);
  					                        }
									...
									$.ajax({
										  url: '".$dominio."/painel/competentes/novo/includes/salvaimgfoto.inc.php',
										  success: function(salvafoto) {
											  $('#fotoimg').attr('src','".$dominio."/painel/competentes/foto/'+salvafoto+'?".rand(1, 999)."');
										  }
									  });
									  ...
									  $('#remove_img_foto').on('click',function() {
										  ...
										  $.ajax({
											  url: '".$dominio."/painel/competentes/novo/includes/unsetfoto.inc.php'
										  });
									  });
...
<script>
	...
	$('#voltar').on('click',function() {
		competenteFundamental(<?php echo $competente['vid'] ?>);
	});
</script>
