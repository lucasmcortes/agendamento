	...
	<corpo>
		...
		<div class='conteudo'>
		        <div style='min-width:100%;max-width:100%;text-align:center;'>
		                <?php
					tituloPagina('editar '.$competente['modelo']);

					//Icone('relatoriocompetente','relatório','relatorioicon');
					//Icone('vercalendario','calendário','calendarioicon');
					Icone('verdoc','documento','verdocicon');
					Icone('verfoto','foto','verfotoicon');
					Icone('removecompetente','status de operação','removecompetenteicon');
				?>
							...
							<div class="items">
								...
								<div id='categoriawrap' style='min-width:100%;max-width:100%;margin:3px auto;'>
									...
									<div class='inputouterwrap'>
										<div class='inputwrap'>
					                        ...
					                        <script>
					                                $('#portasswitch').prop('checked', <?php echo ($competente['portas']==2) ? 'false' : (($competente['portas']==2) ? 'false' : 'true') ?>);
								...
								<div id='completowrap' style='min-width:100%;max-width:100%;margin:3px auto;margin-bottom:7px;display:inline-block;'>
									...
									<div class='inputouterwrap'>
										<div class='inputwrap'>
					                        ...
					                        <script>
					                                $('#completoswitch').prop('checked', <?php echo ($competente['completo']=='S') ? 'true' : 'false'; ?>);
									...
									<div class='inputouterwrap'>
										<div class='inputwrap'>
											...
											<div class='posinput'></div>
				                                ...
				                                <script>
					                                $('#limpezaswitch').prop('checked', <?php echo ($competente['limpeza']=='S') ? 'true' : 'false'; ?>);
								...
								<div id='observacaowrap' style='min-width:100%;max-width:100%;margin:3px auto;'>
									...
									<div class='inputouterwrap'>
										<div class='inputwrap'>
											<div class='preinput normal'></div>
											<textarea class='wrappedinput' id='observacao' rows='5' style='height:120px;'><?php echo $competente['observacao'] ?></textarea>
											<div class='posinput'></div>
										</div>
									</div>
								</div> <!-- observacaowrap -->
		...
		<script>
			$('#relatoriocompetente').on('click',function () {
				window.location.href='<?php echo $dominio ?>/painel/competentes/relatorio/?v=<?php echo $competente['vid'] ?>';
			});

			$('#verdoc').on('click',function () {
				verDoc(<?php echo $competente['vid'] ?>);
			});

			$('#verfoto').on('click',function () {
				verFoto(<?php echo $competente['vid'] ?>);
			});

			$('#removecompetente').on('click',function () {
				$.ajax({
					type: 'POST',
					url: '<?php echo $dominio ?>/painel/competentes/editar/includes/removercompetentepopup.inc.php',
					data: {
			...
			$('#vercalendario').on('click',function () {
				calendarioPop(3,'vestimenta',<?php echo $competente['vid'] ?>);
			});

			$('#modelo').val('<?php echo $competente['modelo'] ?>');
			$('#marca').val('<?php echo $competente['marca'] ?>');
			$('#potencia').val('<?php echo $competente['potencia'] ?>');
			$('.categorias[id=cat_<?php echo $competente['categoria'] ?>]').addClass('selecionada');
			$('#cor').val('<?php echo $competente['cor'] ?>');
			$('#ano').val('<?php echo $competente['ano'] ?>');
			$('#placa').val('<?php echo $competente['placa'] ?>');
			$('#chassi').val('<?php echo mb_strtoupper($competente['chassi']) ?>');
			$('#renavam').val('<?php echo $competente['renavam'] ?>');
			$('#kilometragem').val('<?php echo $competente['km'] ?>');
			...
			$('.posinput').on('click', function() {
				...
				$.ajax({
					type: 'POST',
					url: '<?php echo $dominio ?>/painel/competentes/includes/v'+elemento+'mod.inc.php',
					data: {
