	...
	if (isset($_SESSION['u_id'])) {
		...
		if ( (isset($_GET['v'])) && (is_numeric($_GET['v'])) ) {
			...
			if ($competente['vid']!=0) {
				echo "
					<script>
						document.title = 'Relatório do competente ".$competente['modelo']."';
					</script>
				";
				...
				$anominimo = new DateTime($competente['data']);
				...
				$cadastro = new DateTime($competente['data']);
				($competente['completo']=='S') ? $completo = 'Sim' : $completo = 'Não';
				...
				$listaalugueis = $listaalugueis->ListaAlugueiscompetente($vid);
				...
				$listamanutencoes = $listamanutencoes->competenteManutencoes($vid);
					...
					foreach ($listamanutencoes as $manutencao) {
						...
						if ($retorno['rid']!=0) {
							...
							if ( ($data_inicio->format('Y-m-d')>=$data_inicial_periodo->format('Y-m-d')) && ($data_devolucao->format('Y-m-d')<=$data_devolucao_periodo->format('Y-m-d')) ) {
								...
								$manutencaomotivo = $manutencaomotivo->competenteManutencao($manutencao['motivo']);
		...
		<div class='conteudo' style='min-height:34vh;'>
		        ...
		        <div style='min-width:100%;max-width:100%;text-align:center;'>
					...
					<div style='min-width:90%;max-width:90%;display:inline-block;margin:0 auto;padding:1.2% 3%;'>
						<?php tituloPagina($competente['modelo']); ?>
						<div style='min-width:100%;max-width:100%;display:inline-block;margin:1.8% auto;'>
							<div class='relatorio'>
								<p class='relatorio'><b>Marca:</b> <?php echo $competente['marca']?:'-' ?> </p>
								<p class='relatorio'><b>Cor:</b> <?php echo $competente['cor'] ?> </p>
								<p class='relatorio'><b>Ano:</b> <?php echo $competente['ano'] ?> </p>
							</div>
							<div class='relatorio'>
							...
							<div class='relatorio'>
								<p class='relatorio'><b>Placa:</b> <?php echo $competente['placa'] ?> </p>
								<p class='relatorio'><b>Chassi:</b> <?php echo $competente['chassi']?:0 ?> </p>
								<p class='relatorio'><b>Renavam:</b> <?php echo $competente['renavam']?:0 ?> </p>
							</div>
							<div class='relatorio'>
						...
						<!-- container -->
							...
							<div class="items">
								...
								<!-- container -->
									...
									<?php
										...
										foreach ($concluidos as $concluido) {
											...
											($pagoaposfechamento<0) ? $devolvido = ' devolvido(s) ao cliente' : $devolvido = '';
												...
												<div id='relatoriofaturamento_wrap_".$concluido['aid']."' class='relatoriowrap aluguel'>
													<div class='slotrelatoriowrap'>
														<div class='slotrelatorio'>
															...
															<p class='headerslotrelatorio'>
																<span class='info' aria-label='data de devolução do competente'>
														...
														<div class='slotrelatorio'>
															...
															<p class='infoslotrelatorio'>".$concluido['diarias']."/".$concluido['cortesias']."</p>
																<p class='headerslotrelatorio'>
																	<span class='info' aria-label='cliente associado/acionamento da placa'>
								...
								<?php tituloRelatorio('saldo do competente'); ?>
								...
								<div class='relatorio'>
									<p class='relatorio' style='float:left;'>• <b>Saldo do competente:</b></p>
