		...
		<div id='altassociado' style='min-width:100%;max-width:100%;display:inline-block;'>
			<?php
				...
				<div id='associadowrap' style='min-width:100%;max-width:100%;'>
					<span id='associadoinfo' class='info' aria-label='"; echo ($cliente['associado']=='S') ? 'Associado' : 'Associar'; echo "'>
				";
				...
				<script>
					$('#associadoswitch').prop('checked', "; echo (($cliente['associado']=='S') ? 'true' : 'false'); echo ");
					$('#associadoswitch').on('change',function() {
						...
						/*$.ajax({
							type: 'POST',
							url: '".$dominio."/painel/clientes/includes/lassociadomod.inc.php',
							data: {
								cliente: lid,
							...
							success: function(modassociado) {
								if (modassociado.includes('sucesso')) {
									...
									$('#associadoswitch').prop('checked', "; echo (($cliente['associado']=='S') ? 'true' : 'false'); echo ");
									$('#associadoinfo').attr('aria-label', '"; echo ($cliente['associado']=='S') ? 'Associado' : 'Associar'; echo "');
...
<script>
	...
	$('#enviaraltassociado').on('click',function() {
		...
		$.ajax({
			type: 'POST',
			url: '<?php echo $dominio ?>/painel/clientes/includes/lassociadomod.inc.php',
			data: {
				cliente: lid,
				associado: lassociado,
				pwd: valpwd
	...
	$('#enviarplaca').on('click',function() {
		...
		$.ajax({
			type: 'POST',
			url: '<?php echo $dominio ?>/painel/clientes/includes/addplaca.inc.php',
			data: {cliente: lid},
	...
	$('#removerplaca').on('click',function() {
		...
		$.ajax({
			type: 'POST',
			url: '<?php echo $dominio ?>/painel/clientes/includes/removerplaca.inc.php',
			data: {cliente: lid},
			success: function(linfo) {
				$('#vestimenta').html(linfo);
			}
