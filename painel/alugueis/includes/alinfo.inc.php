...
<div class="items">
	...
	<div style='display:flex;flex-direction: column;'>
		<p><?php echo $competente['placa'].' - '.$limpeza ?></p>
		<p class='numregistro'>
				...
				if ($cobranca['tid']!=0) {
					...
					if ($pagoateomomento>0) {
						...
						if (($valor_mostrado)>0) {
							echo "
								<div style='min-width:100%;max-width:100%;display:inline-block;margin-top:3%;'>
								<p style='display:inline-block;' class='bloquinho'>Totalizando <b>".Dinheiro(str_replace('-','',$valor_mostrado))."</b> a serem pagos pelo cliente</p>
								</div>
							";
						} else {
							$faturaok = 1;
							echo "
								<div style='min-width:100%;max-width:100%;display:inline-block;margin-top:3%;'>
									<p style='display:inline-block;' class='bloquinho'>Totalizando <b>".Dinheiro(str_replace('-','',$valor_mostrado))."</b> devolvidos ao cliente</p>
								</div>
							";
						} // valor_mostrado > 0
					} // valorinicial > 0
				...
				if ($valorinicial>0) {
					if (isset($cobranca)) {
						if (($cobranca['valor']-$valorinicial)<0) {
							...
							echo "
								<div style='min-width:100%;max-width:100%;display:inline-block;margin-top:3%;'>
									<p style='display:inline-block;'><b>".Dinheiro(str_replace('-','',$cobranca['valor']-$pagamentosaluguel+$somaparciais))." devolvidos ao cliente</b></p>
								</div>
							";
						} // valor_mostrado > 0
					} // se existe cobrança
	...
	<?php
		...
		if ($_POST['ativo']!=0) {
			...
			if ($inicio<$agora) {
				Icone('devolvercompetente','devolver','devolvericon');
...
<script>
	...
	$('#devolvercompetente').on('click',function () {
	...
	$('#vinfo').on('click', function() {
		competenteFundamental(<?php echo $aluguel['vid'] ?>);
	});

	$('#linfo').on('click',function () {
		clienteFundamental(<?php echo $aluguel['lid'] ?>);
	});
